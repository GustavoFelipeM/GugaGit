...
def interpretar_erro_git(comando: List[str], stderr: str) -> str:
    """Mapeia erros técnicos do Git para mensagens amigáveis."""
    ...
    if cmd_principal == "push" and (
        "fetch first" in err_lower or "non-fast-forward" in err_lower
    ):
        return (
            "Push rejeitado: Existem alterações no servidor remoto. Faça um"
            " 'Pull' antes de enviar."
        )

    if (
        cmd_principal == "checkout"
        and "local changes to the following files would be overwritten"
        in err_lower
    ):
        return (
            "Troca de branch cancelada: Salve ou descarte suas alterações"
            " locais antes de mudar de branch."
        )

    if "could not resolve host" in err_lower:
        return (
            "Erro de Conexão: Sem acesso à internet ou ao servidor Git remoto."
        )
...
def executar_e_tratar(comando: List[str], msg_sucesso: str) -> Tuple[str, bool, str]:
    ...
    try:
        ...
        return str(exc), False, "Git não instalado no sistema."
...
def executar_comando_livre(comando_texto: str, pasta_repositorio: Optional[str] = None) -> Tuple[str, bool]:
    ...
    try:
        ...
        logger.exception(f"Falha inesperada ao executar comando livre: {comando_texto}")
        return f"ERRO ao executar comando: {str(exc)}", False